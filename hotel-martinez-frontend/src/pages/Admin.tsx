  ...
  const addImage = () => {
    if (newImageUrl.trim() && formData) {
      ...
      setFormData({
        ...formData,
        images: newImages,
        image: newImages[0] // Первое изображение становится основным
      })
      setNewImageUrl('')
    }
  }
  ...
  const deleteImage = (index: number) => {
    if (formData) {
      ...
      setFormData({
        ...formData,
        images: newImages,
        image: newImages.length > 0 ? newImages[0] : formData.image // Если осталось - первое из оставшихся, иначе оставляем старое
      })
    }
  }
